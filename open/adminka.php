...
while ($row = $admins->fetch(PDO::FETCH_NUM)) 
{
		...
		if($row[3] != "1"){	echo "<td><a href='setAdmin.php?id={$row[0]}&admin=1'>сделать админом</a></td>"; }else{ echo "<td><a href='setAdmin.php?id={$row[0]}&admin=0'>разжаловать</a></td>";};
...
while ($tema = $viborKategorii->fetch(PDO::FETCH_NUM)) 
	{
		...
		echo "<td>".$fre."</td>";
		echo "<form action='setTema.php' method='POST'>";
...
<?php
	...
	while ($basePole = $baseCategory->fetch(PDO::FETCH_NUM)) 
	{
		...
		echo "<form action='setQuestion.php' method='POST' >";
